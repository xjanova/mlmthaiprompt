@if($project && $currentWorkspace && $task)
        {{ Form::model($task, array('route' => array('tasks.update',[$project->id,$task->id]), 'method' => 'Post','class'=>'needs-validation','novalidate')) }}
        @csrf
        ...
        {{ Form::close() }}
    <link rel="stylesheet" href="{{ asset('addon/Taskly/src/Resources/assets/libs/bootstrap-daterangepicker/daterangepicker.css')}} ">

    <script src="{{ asset('addon/Taskly/src/Resources/assets/libs/moment/min/moment.min.js')}}"></script>
    <script src="{{ asset('addon/Taskly/src/Resources/assets/libs/bootstrap-daterangepicker/daterangepicker.js')}}"></script>
